...
#define PLAN_MODEL_PATH     "./models/temp.plan"
...
//    engine = trt_load_plan_file (model_name);

    /* Build TensorRT Engine */
    if (engine == NULL)
    {
        convert_onnx_to_plan (PLAN_MODEL_PATH, model_name);
    ...
    printf("finish model converting from onnx to plan\n");
    s_trt_context = engine->createExecutionContext();


    /* Allocate IO tensors */
    trt_get_tensor_by_name (engine, "data",       &s_tensor_input);   /* (1, 256, 448,  3) */
    trt_get_tensor_by_name (engine, "Identity/Conv2D",   &s_tensor_offsets); /* (1,  32,  56, 57) */
    trt_get_tensor_by_name (engine, "Identity_1/Conv2D", &s_tensor_heatmap); /* (1,  32,  56  19) */
