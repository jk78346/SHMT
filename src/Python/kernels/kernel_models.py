...
class KernelModels:
    ...
    def laplacian_2d(in_shape, out_shape):
        ...
        encoded_dim = 16
        inputs = keras.Input(shape=in_shape+(1,))
        x = layers.Conv2D(filters=encoded_dim, kernel_size=(5, 5), padding='same', activation='relu')(inputs)
        x = layers.Conv2D(filters=encoded_dim, kernel_size=(5, 5), padding='same', activation='relu')(x)
        x = layers.Conv2D(filters=1, kernel_size=(5, 5), padding='same', activation='relu')(x)
        outputs = x
        return keras.Model(inputs, outputs)
